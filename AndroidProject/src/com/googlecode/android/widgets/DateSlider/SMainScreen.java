...
public class SMainScreen extends Activity implements SimpleGestureListener
{
	Dialog dialog,poss_att;
	TextView bunk_display;
	int curr_attended=0;
	int curr_total=0;
	int classtoday=0;
	int classnottoday=0;
	...
	@Override
	 protected void onCreate(Bundle savedInstanceState) {
		...
		Intent i=new Intent(SMainScreen.this,Refresh.class);
		i.putExtras(b);
		startActivity(i);
	   ...
	   //    listAdapter = new ArrayAdapter<String>(this, android.R.layout.simple_list_item_1, option);
	 	...
	 	Bundle b=getIntent().getExtras();
			...
			classdates=new Date[100];
			classdates2=new Date[100];
			classdates3=new Date[100];
	...
	/*	attended=b.getStringArray("attended");
	 	...
	 	System.out.println(c3.getCount()+"columns");
	 	int column_count=c3.getColumnCount();
	 	if(c3.moveToFirst())
	 		do
	 		{
	 	for( i=0;i<column_count-1;i++)
	 	{
	 		String status=c3.getString(i+1);
	 		if(!(status==null))
	 		{
	 			if(status.equalsIgnoreCase("Present") || status.equalsIgnoreCase("On Duty"))
	 			{
	 			int x=Integer.parseInt(attended[i]);	
	 			x++;
	 			attended[i]=Integer.toString(x);
	 			int y=Integer.parseInt(total_class[i]);	
	 			y++;
	 			total_class[i]=Integer.toString(y);
	 			}
	 		...
	 		{
	 		int z=x*100/y;
	 		course_percent[i]=Integer.toString(z);
	 		}
	 		else
	 		course_percent[i]="-";
	 	}
	 	...
	 	stud.close();
	 	for( i=0;i<c2.getCount();i++)
	 	{
	 		System.out.println(attended[i]+"/"+total_class[i]);
	 	}
...
//	calculate_days();
	try {
		...
		e.printStackTrace();
		 ...
		 menulist=(ListView) dialog.findViewById(R.id.dialog_menu);
 ...
 public void settimetable(int day)
 {
	 		...
	 		if(c.moveToFirst())
			{
				...
				if(!(Integer.parseInt(total_class[i])==0))
    		 		
    		 	{
    		 		double w1=((double) Integer.parseInt(attended[i])*100/(double) Integer.parseInt(total_class[i]));
    		 		double w2=(Integer.parseInt(attended[i])*100/Integer.parseInt(total_class[i]));
	    		 	
    		 		double x=w2+1.0;
    		 		if(w1==w2)
    		 			course_percent[i]=Double.toString(w2);
    				
    		 		else
    		 		 course_percent[i]=Double.toString(x);
	 		...
	 		{
	 			System.out.println("hhhh"+course_slot[count]);
	 		...
	 		{
	 			System.out.println(marks_today[l]);
	 		}
	 		
	 		System.out.println("not today classes");
	 		for(int l=0;l<nottoday;l++)
	 		{
	 			System.out.println(marks_nottoday[l]);
	 		}
